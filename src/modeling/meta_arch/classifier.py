...
class Classifier(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, Any]]):
        logits, probas, cat_features = self.get_probas(batched_inputs)
        results = self.format_result(batched_inputs, logits, probas, cat_features)
        return results
    ...
    def get_probas(self, batched_inputs: List[Dict[str, Any]],
        ...
        return logits, probas, cat_features
    ...
    def format_result(self, batched_inputs: List[Dict[str, Any]],
                      logits: torch.tensor, probas: torch.tensor, features: torch.tensor) -> Dict[str, Any]:
        ...
        if isinstance(batched_inputs, list):
            ...
            raise ValueError("Input format not recognized.")
        results = {'logits': logits, 'probas': probas, 'features': features}
        if not is_tensor:
